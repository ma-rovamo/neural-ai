...
export default function ActionBar() {
	return (
		<div className="bg-[#232323] rounded-3xl p-4 md:p-6  mx-auto shadow-lg border border-gray-800">
					...
					<div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
						...
						<div className="w-full lg:flex-shrink-0 lg:max-w-xs">
							...
							<p className="text-gray-400 text-sm leading-relaxed mb-4">
								Amazon.com, Inc. offers a range of product and services through
								its websites
							</p>
						...
						<div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 mt-4 lg:mt-0">
							{metrics.map((metric, index) => (
								<div
									...
									<div
										className={`${metric.textColor} text-xl md:text-2xl font-bold mb-1 whitespace-nowrap`}>
										{metric.value}
									</div>
									<div
										className={`${metric.textColor} text-xs opacity-80 whitespace-nowrap`}>
										{metric.label}
									</div>
								</div>
							))}
